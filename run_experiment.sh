...
#SBATCH --output=logs.cifar10_resnet18_nobias_nobn_unstructured_pruning_70.out
...
python3 main.py --gpu-id 0 --model-name resnet18_nobias_nobn --n-epochs 300 --save-result-file sample_cifar10_resnet18_nobias_nobn_unstructured_pruning_70.csv \
--sweep-name exp_cifar10_resnet18_nobias_nobn_unstructured_pruning_70 --correction --ground-metric  euclidean --weight-stats \
--activation-histograms --activation-mode raw --geom-ensemble-type acts --act-num-samples 200 --ground-metric-normalize none --sweep-id 90 --ckpt-type best --to-download --dataset Cifar10 \
--ground-metric-eff --recheck-cifar --recheck-acc --activation-seed 21 --prelu-acts --past-correction --not-squared \
--exact --experiment-name cifar10_resnet18_nobias_nobn_unstructured_pruning_70 --retrain 300 --retrain-geometric-only --load-models ./resnet_models/ --handle-skips \
--prune --prune-frac 0.7 --prune-type unstructured
