...
config = {
    ...
    'parent2_cp_path': '',
    # DO NOT SET MANUALLY
    'fusion_inital_cp_path': '',
    # DO NOT SET MANUALLY
    'fusion_retrained_cp_path': '',
    'device_id' : 0,
    'use_cuda' : True,
    'batch_size': 10, # 1000 = value as in original code of sidak pal
    'num_batches': 2,  # -1 = all batches in training set
    'top_ev': 3, # quickly consumes much time (for higher values)
    'lambdas': np.linspace(-0.5, 0.5, 21).astype(np.float32).tolist(),
    'compute_ev_density': False # set to true for plotting eigenvalue density
}
...
for exp in experiments:
    ...
    config['fusion_inital_cp_path'] = f"{path}/geometric/fused.initial.checkpoint"
